...
- (void)objectDidLoad{
  ...
  if ([objectDict objectForKey:@"addButton"]) {
    ...
    [self.navigationItem performSelectorOnMainThread:@selector(setRightBarButtonItem:)
                                          withObject:addButton
                                       waitUntilDone:NO];
  }
}
...
- (void)addItem:(id)sender{
  if (objectDict[@"addButton"]) {
    ...
    if ([objectDict[@"addButton"] count] > 1) {
      ...
      [apiManager openURL:[apiManager URLForResourceAtPath:objectDict [@"addButton"][0][@"path"]]
        ...
        if (size.width > label.bounds.size.width) {
          ...
          UIAlertView* alert = [[UIAlertView alloc]
                      ...
                      [object[@"leftItems"][[indexPath row]] stringByReplacingOccurrencesOfString:@":" withString:@""]]
                                      message:[NSString stringWithFormat:@"%@",object[@"rightItems"][[indexPath row]]]
                                     delegate:self
